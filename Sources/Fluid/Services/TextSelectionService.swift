...
final class TextSelectionService {
    ...
    /// Reads the full text value of the focused element in the app with the
    /// given PID. Used by correction learning to re-read the field Speech2Write
    /// Flow typed into. Returns nil when Accessibility permission is missing
    /// or the app exposes no focused text element.
    func focusedElementText(forPID pid: pid_t) -> String? {
        guard AXIsProcessTrusted() else {
            self.diag("Focused element text read failed: Accessibility permissions not granted")
            return nil
        }

        let appElement = AXUIElementCreateApplication(pid)
        guard let focusedElement = self.getFocusedElement(from: appElement) else {
            self.diag("Focused element text read failed: no focused element for pid=\(pid)")
            return nil
        }

        var valueRef: CFTypeRef?
        let result = AXUIElementCopyAttributeValue(focusedElement, kAXValueAttribute as CFString, &valueRef)
        guard result == .success, let text = valueRef as? String else {
            self.diag("kAXValueAttribute unavailable for pid=\(pid) (\(self.describe(result)))")
            return nil
        }

        self.diag("Focused element text read succeeded (chars=\(text.count))")
        return text
    }
