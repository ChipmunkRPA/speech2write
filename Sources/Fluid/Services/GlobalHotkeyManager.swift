...
final class GlobalHotkeyManager: NSObject {
    ...
    private var primaryShortcutSet: HotkeyShortcutSet
    ...
    init(
        ...
        self.primaryShortcutSet = HotkeyShortcutSet(primaryShortcuts)
    ...
    func updatePrimaryShortcuts(_ newShortcuts: [HotkeyShortcut]) {
        let replacement = HotkeyShortcutSet(newShortcuts)
        guard replacement != self.primaryShortcutSet else { return }
        self.resetModifierOnlyShortcutTracking(reason: .configurationChanged)
        self.primaryShortcutSet = replacement
    ...
    func updateRewriteModeShortcut(_ newShortcut: HotkeyShortcut) {
        guard newShortcut != self.rewriteModeShortcut else { return }
        self.resetModifierOnlyShortcutTracking(reason: .configurationChanged)
    ...
    func updateRewriteModeShortcutEnabled(_ enabled: Bool) {
        guard enabled != self.rewriteModeShortcutEnabled else { return }
        self.resetModifierOnlyShortcutTracking(reason: .configurationChanged)
    ...
    func updatePromptModeShortcut(_ newShortcut: HotkeyShortcut) {
        guard newShortcut != self.promptModeShortcut else { return }
        self.resetModifierOnlyShortcutTracking(reason: .configurationChanged)
    ...
    func updatePromptModeShortcutEnabled(_ enabled: Bool) {
        guard enabled != self.promptModeShortcutEnabled else { return }
        self.resetModifierOnlyShortcutTracking(reason: .configurationChanged)
    ...
    private func handleKeyEvent(proxy: CGEventTapProxy, type: CGEventType, event: CGEvent) -> Unmanaged<CGEvent>? {
        ...
        case .keyDown:
            ...
            if let shortcut = self.primaryShortcutSet.keyboardShortcut(keyCode: keyCode, modifiers: eventModifiers) {
        ...
        case .flagsChanged:
            ...
            for shortcut in self.primaryShortcutSet.modifierOnlyShortcuts {
                if self.handleModifierOnlyShortcutFlagsChanged(
    ...
    private func handleMouseShortcutDown(_ event: CGEvent, modifiers eventModifiers: NSEvent.ModifierFlags) -> Bool {
        ...
        if self.primaryShortcutSet.containsMouseShortcut(button: mouseButton, modifiers: eventModifiers) {
