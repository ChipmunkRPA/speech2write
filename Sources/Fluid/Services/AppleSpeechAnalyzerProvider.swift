    ...
    func transcribe(_ samples: [Float]) async throws -> ASRTranscriptionResult {
        ...
        let resultsTask = Task {
            ...
            for try await case let result in freshTranscriber.results {
                ...
                DebugLogger.shared.debug(
                    "AppleSpeechAnalyzer: Got result - isFinal: \(result.isFinal), content omitted, chars=\(text.count)",
                    source: "AppleSpeechAnalyzerProvider"
                )
                if result.isFinal {
            ...
            DebugLogger.shared.debug(
                "AppleSpeechAnalyzer: Results iteration complete [content omitted, chars=\(finalText.count)]",
                source: "AppleSpeechAnalyzerProvider"
            )
        }
        ...
        DebugLogger.shared.debug(
            "AppleSpeechAnalyzer: Transcription complete [content omitted, chars=\(finalText.count)]",
            source: "AppleSpeechAnalyzerProvider"
        )
