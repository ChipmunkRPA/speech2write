...
final class ParakeetVocabularyStore {
    ...
    private enum BrandVocabulary {
        static let currentText = "Speech2Write"
        static let currentAliases = ["speech2write", "speech to write", "speech two write"]
        static let legacyText = "Platypus Flow"
        static let legacyAliases = Set(["platypus flow", "platypus", "platy pus flow"])
        static let weight: Float = 10.0
        ...
        static let requiredTerms = [
            ...
            VocabularyConfig.Term(
                text: "CPA Automation",
                weight: weight,
                aliases: ["C.P.A. Automation", "CPA automation"]
            ),
            VocabularyConfig.Term(
                text: "cpaautomation.ai",
                weight: weight,
                aliases: ["CPA Automation dot AI", "CPA automation AI"]
            ),
        ]
    }
    ...
    func ensureVocabularyFileExists() throws -> URL {
        ...
        if FileManager.default.fileExists(atPath: url.path) {
            do {
                try self.migrateBrandVocabularyIfNeeded(at: url)
            } catch {
                DebugLogger.shared.warning(
                    "ParakeetVocabularyStore: could not migrate product vocabulary terms",
                    source: "ParakeetVocabularyStore"
                )
            }
            return url
    ...
    private func migrateBrandVocabularyIfNeeded(at url: URL) throws {
        let data = try Data(contentsOf: url)
        guard let config = try? JSONDecoder().decode(VocabularyConfig.self, from: data) else { return }
        let legacyMigration = Self.migratingLegacyBrandTerm(in: config)
        let brandMigration = Self.addingRequiredBrandTerms(in: legacyMigration.config)
        guard legacyMigration.didChange || brandMigration.didChange else { return }

        let encoder = JSONEncoder()
        encoder.outputFormatting = [.prettyPrinted, .sortedKeys]
        try encoder.encode(brandMigration.config).write(to: url, options: .atomic)
        DebugLogger.shared.info(
            "ParakeetVocabularyStore: migrated product vocabulary terms",
            source: "ParakeetVocabularyStore"
        )
    }

    static func addingRequiredBrandTerms(
        in config: VocabularyConfig
    ) -> (config: VocabularyConfig, didChange: Bool) {
        var terms = config.terms
        var didChange = false

        for required in BrandVocabulary.requiredTerms {
            if let index = terms.firstIndex(where: {
                $0.text.caseInsensitiveCompare(required.text) == .orderedSame
            }) {
                let existing = terms[index]
                let aliases = Array(Set(existing.aliases + required.aliases)).sorted()
                let weight = max(existing.weight ?? 0, required.weight ?? 0)
                let updated = VocabularyConfig.Term(
                    text: required.text,
        ...
        return (
            VocabularyConfig(
                alpha: config.alpha,
                minCtcScore: config.minCtcScore,
    ...
    static func migratingLegacyBrandTerm(
        in config: VocabularyConfig
    ) -> (config: VocabularyConfig, didChange: Bool) {
        let firstLegacyIndex = config.terms.firstIndex {
            $0.text.caseInsensitiveCompare(BrandVocabulary.legacyText) == .orderedSame
        }
        guard let firstLegacyIndex else { return (config, false) }

        let legacyTerms = config.terms.filter {
            $0.text.caseInsensitiveCompare(BrandVocabulary.legacyText) == .orderedSame
        }
        let legacyWeight = legacyTerms.compactMap(\.weight).max() ?? 0
        var terms = config.terms.filter {
            $0.text.caseInsensitiveCompare(BrandVocabulary.legacyText) != .orderedSame
        }

        if let currentIndex = terms.firstIndex(where: {
            $0.text.caseInsensitiveCompare(BrandVocabulary.currentText) == .orderedSame
        }) {
            let current = terms[currentIndex]
            let retainedAliases = current.aliases.filter {
                !BrandVocabulary.legacyAliases.contains($0.lowercased())
            }
            let aliases = Array(Set(retainedAliases + BrandVocabulary.currentAliases)).sorted()
            terms[currentIndex] = VocabularyConfig.Term(
                text: BrandVocabulary.currentText,
                weight: max(BrandVocabulary.weight, max(current.weight ?? 0, legacyWeight)),
                aliases: aliases
            )
        } else {
            terms.insert(
                VocabularyConfig.Term(
                    text: BrandVocabulary.currentText,
                    weight: max(BrandVocabulary.weight, legacyWeight),
                    aliases: BrandVocabulary.currentAliases
                ),
                at: min(firstLegacyIndex, terms.count)
            )
        }
    ...
    func loadResolvedConfig() throws -> ResolvedConfig {
        ...
        let enabledPresetIDs = SettingsStore.shared.enabledVocabularyPresetIDs
        let presetTerms = VocabularyPresetCatalog.terms(for: enabledPresetIDs)
        let mergedTerms = Self.mergeAndNormalizeTerms(
            jsonTerms: parsed.terms,
            dictionaryEntries: SettingsStore.shared.customDictionaryEntries,
            presetTerms: presetTerms
        )
        DebugLogger.shared.debug(
            "ParakeetVocabularyStore: merged terms=\(mergedTerms.count), dictionaryEntries=\(SettingsStore.shared.customDictionaryEntries.count), presets=\(enabledPresetIDs.count)",
            source: "ParakeetVocabularyStore"
        )
    ...
    static func defaultTemplateJSON() -> String {
        """
        {
          ...
          "terms": [
            {
              "text": "Speech2Write",
              "aliases": ["speech2write", "speech to write", "speech two write"],
              "weight": 10.0
            },
            {
              "text": "CPA Automation",
