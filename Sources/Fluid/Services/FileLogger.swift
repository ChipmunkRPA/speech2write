...
final class FileLogger {
    ...
    /// Diagnostic logs can contain app names, file paths, and operational metadata. Keep the
    /// directory and both generations of the log private to the current macOS account.
    private func hardenLogPermissions() {
        try? self.fileManager.setAttributes([.posixPermissions: 0o700], ofItemAtPath: self.logDirectory.path)
        for url in [self.logFileURL, self.backupLogURL, self.legacyLogFileURL, self.legacyBackupLogURL]
            where self.fileManager.fileExists(atPath: url.path)
        {
