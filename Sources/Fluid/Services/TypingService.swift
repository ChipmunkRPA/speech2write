...
final class TypingService {
    ...
    private static let pendingReturnKeyQueue = DispatchQueue(label: "TypingService.PendingReturnKey")
    private static var pendingReturnKeyAfterInsertion = false
    ...
    // MARK: - Secure input target detection (SR-5829 REQ-7)

    /// PID of the process holding the system secure-input latch, read from the
    /// current console session in the IORegistry root's IOConsoleUsers array
    /// (the value `ioreg` reports as kCGSSessionSecureInputPID). nil when the
    /// latch is clear or the registry is unreadable.
    nonisolated static func secureInputHolderPID() -> pid_t? {
        ...
        for session in users {
            ...
            let sessionUID = session["kCGSSessionUserIDKey"] as? UInt32
            guard onConsole, sessionUID == uid else { continue }
            guard let pid = session["kCGSSessionSecureInputPID"] as? Int else { return nil }
            return pid_t(pid)
        }
        return nil
    }

    /// True when the app we are about to type into is in a secure input
    /// context: its focused UI element is a secure text field, or the app is
    /// itself the process holding the secure-input latch. A latch held by an
    /// unrelated background process (stuck loginwindow, Terminal Secure
    /// Keyboard Entry) does NOT make an ordinary target secure.
    nonisolated static func isSecureTypingTarget(_ targetPID: pid_t?) -> Bool {
        guard let pid = targetPID else {
            // Secure input is active and we cannot identify the target: fail closed.
            return true
        }
        if let holder = self.secureInputHolderPID(), holder == pid {
            return true
        }
        let appElement = AXUIElementCreateApplication(pid)
    ...
    // MARK: - Synthetic Return key ("press enter" auto-send)

    /// Arms a one-shot synthetic Return keypress that fires after the next text
    /// insertion completes. Consumed (and discarded) if that insertion aborts.
    static func requestReturnKeyAfterNextInsertion() {
        self.pendingReturnKeyQueue.sync { Self.pendingReturnKeyAfterInsertion = true }
    }

    private static func consumePendingReturnKeyRequest() -> Bool {
        self.pendingReturnKeyQueue.sync {
            let pending = Self.pendingReturnKeyAfterInsertion
            Self.pendingReturnKeyAfterInsertion = false
            return pending
        }
    }

    /// Posts a synthetic Return key press, targeting a specific PID when available.
    /// Mirrors the Cmd+V event routing used for paste insertion.
    static func postReturnKeystroke(targetPID: pid_t?) {
        let returnKey = CGKeyCode(kVK_Return)
        guard let keyDown = CGEvent(keyboardEventSource: nil, virtualKey: returnKey, keyDown: true),
              let keyUp = CGEvent(keyboardEventSource: nil, virtualKey: returnKey, keyDown: false)
        else {
            return
        }
    ...
    func typeTextInstantly(_ text: String, preferredTargetPID: pid_t?, textReadyAt: TimeInterval?) {
        ...
        self.log("[TypingService] Text preview omitted [chars=\(text.count)]")

        guard text.isEmpty == false else {
            _ = Self.consumePendingReturnKeyRequest()
        ...
        // Never type into a secure input context (password fields). Synthetic
        // keystrokes would route dictated secrets through the pasteboard and
        // transcript history (SR-5829 REQ-7).
        //
        // The check is TARGET-AWARE: the global secure-input flag alone is not
        // enough, because any background process can hold the latch system-wide
        // (a stuck loginwindow after screen lock, Terminal's Secure Keyboard
        // Entry, password managers) — blocking on the flag alone bricks
        // dictation everywhere. We block only when the app we are about to
        // type into is the secure one: its focused element is a secure text
        // field, or it is itself the process holding the secure-input latch.
        if IsSecureEventInputEnabled() {
            let targetPID = preferredTargetPID
                ?? NSWorkspace.shared.frontmostApplication?.processIdentifier
            if Self.isSecureTypingTarget(targetPID) {
                _ = Self.consumePendingReturnKeyRequest()
                self.bench("request_return reason=secure_input")
                self.log("[TypingService] Secure input field focused in target — dictation not typed")
                NotificationService.showSecureInputBlocked()
                return
            }
            let holder = Self.secureInputHolderPID().map(String.init) ?? "unknown"
            self.log("[TypingService] Secure input latch held by pid \(holder) (not the typing target) — proceeding")
        }
        ...
        DispatchQueue.global(qos: .userInitiated).async {
            ...
            if Self.consumePendingReturnKeyRequest() {
                // Give the target app a moment to process the inserted text
                // (paste-based insertion in particular) before pressing Return.
                usleep(150_000)
                let returnTargetPID = preferredTargetPID ?? self.getSystemFocusedElementAndPID()?.pid
                Self.postReturnKeystroke(targetPID: returnTargetPID)
                self.log("[TypingService] Posted synthetic Return after insertion")
            }
        }
    }
    ...
    private func insertTextInstantly(_ text: String, preferredTargetPID: pid_t?) {
        ...
        self.log("[TypingService] Attempting to type text [content omitted, chars=\(text.count)]")
