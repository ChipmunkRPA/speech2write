...
final class ASRService: ObservableObject {
    ...
    /// Uses the unified SettingsStore.selectedSpeechModel setting.
    private var transcriptionProvider: TranscriptionProvider {
        ...
        default:
            // Retired engines (whisper*, nemotron*, cohere, qwen, parakeetRealtime) are
            // coerced away by selectedSpeechModel; route any residual value to the
            // platform default engine.
            if SettingsStore.SpeechModel.defaultModel == .appleSpeech {
                return self.getAppleSpeechProvider()
            }
            return self.getFluidAudioProvider()
        }
    ...
    /// This allows file transcription to work with any provider (Parakeet, Apple Speech, etc.)
    var fileTranscriptionProvider: TranscriptionProvider {
        self.transcriptionProvider
    }

    private func currentTranscriptionModelDimensions() -> (provider: String, model: String) {
    ...
    private func getProvider(for model: SettingsStore.SpeechModel) -> TranscriptionProvider {
        ...
        default:
            // Retired engines — route any residual value to the platform default engine.
            if SettingsStore.SpeechModel.defaultModel == .appleSpeech {
                return AppleSpeechProvider()
            }
            return FluidAudioProvider(modelOverride: .parakeetTDTv2, configureWordBoosting: false)
        }
    }
    ...
    func start(forDictionaryTraining: Bool = false) async {
        ...
        self.refreshWordBoostStatus()
        let dims = self.currentTranscriptionModelDimensions()
        ...
        do {
            ...
            // Only start streaming for models that support it
            let model = SettingsStore.shared.selectedSpeechModel
    ...
    func stop(
        ...
        // "yes", "stop") still transcribe. Some engines choke on buffers
        // shorter than 1s; every provider handles silence padding without
        // issue, so we pad unconditionally rather than branching per
        // provider.
        ...
        do {
            ...
            DebugLogger.shared.debug(
                "Transcription completed [content omitted, chars=\(result.text.count), confidence=\(result.confidence)]",
                source: "ASRService"
            )
            DebugLogger.shared.info(
            ...
            DebugLogger.shared.debug(
                "Post-processing completed [content omitted, chars=\(outputText.count)]",
                source: "ASRService"
            )
    ...
    /// - Intel: Apple Speech (legacy SFSpeechRecognizer)
    ///
    /// ## Performance
    ...
    private func processStreamingChunk() async {
        ...
        do {
            ...
            if !newText.isEmpty {
                ...
                DebugLogger.shared.debug(
                    "✅ Streaming update [content omitted, chars=\(updatedText.count), duration=\(String(format: "%.2f", duration))s]",
                    source: "ASRService"
                )
            }
    ...
    /// Applies custom dictionary replacements to transcribed text, then expands
    /// snippet trigger phrases into their stored text blocks.
    ...
    static func applyCustomDictionary(_ text: String) -> String {
        self.applySnippets(self.applyDictionaryReplacements(text))
    }

    /// Replaces trigger words/phrases with their designated replacements.
    private static func applyDictionaryReplacements(_ text: String) -> String {
    ...
    // MARK: - Snippets (Cached Regex)

    /// Cache for compiled snippet trigger regexes.
    /// Cleared when the snippet list changes.
    private static var cachedSnippetPatterns: [(regex: NSRegularExpression, template: String)] = []
    private static var snippetCacheNeedsRebuild: Bool = true

    /// Rebuilds the snippet regex cache if the snippet list has changed.
    private static func rebuildSnippetCache() {
        let snippets = SnippetStore.shared.snippets
        var patterns: [(regex: NSRegularExpression, template: String)] = []

        for snippet in snippets where snippet.isEnabled {
            guard !snippet.expansion.isEmpty,
                  let pattern = self.snippetTriggerPattern(for: snippet.trigger),
                  let regex = try? NSRegularExpression(pattern: pattern, options: .caseInsensitive)
            else { continue }

            patterns.append((regex: regex, template: NSRegularExpression.escapedTemplate(for: snippet.expansion)))
        }

        self.cachedSnippetPatterns = patterns
        self.snippetCacheNeedsRebuild = false
    }

    /// Builds a pattern that matches the trigger phrase with normalized whitespace
    /// and punctuation, whether it appears alone or inside a sentence. Punctuation
    /// attached directly after the trigger is absorbed so the expansion stays clean.
    private static func snippetTriggerPattern(for trigger: String) -> String? {
        let tokens = trigger
            .lowercased()
    ...
    /// Invalidates the snippet cache. Called when snippets change.
    static func invalidateSnippetCache() {
        self.snippetCacheNeedsRebuild = true
    }

    /// Expands snippet trigger phrases in transcribed text into their stored text blocks.
    static func applySnippets(_ text: String) -> String {
        // Fast path: no snippets configured
        guard !SnippetStore.shared.snippets.isEmpty else { return text }

        // Rebuild cache if needed (lazy initialization)
        if self.snippetCacheNeedsRebuild {
            self.rebuildSnippetCache()
        }

        guard !self.cachedSnippetPatterns.isEmpty else {
            return text
        }

        var result = text

        for pattern in self.cachedSnippetPatterns {
            result = pattern.regex.stringByReplacingMatches(
                in: result,
                range: NSRange(result.startIndex..., in: result),
                withTemplate: pattern.template
