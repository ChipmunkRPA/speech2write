...
struct SettingsBackupPayload: Codable, Equatable {
    ...
    // Command Mode fields removed in 1.2.0 (SR-5829). Old backups that still
    // contain them decode fine — unknown JSON keys are ignored.
    ...
    // Retention window in days (0 = forever); optional so older backups decode.
    ...
    // Optional so backups created before vocabulary presets still decode.
    // swiftlint:disable:next discouraged_optional_collection
    let enabledVocabularyPresetIDs: [String]?
...
enum BackupServiceError: LocalizedError {
    ...
    var errorDescription: String? {
        ...
        case .invalidJSON:
            return "The selected backup file is not a valid Speech2Write backup."
        }
    }
}
...
final class BackupService {
    ...
    func suggestedFilename(for date: Date = Date()) -> String {
        ...
        return "Speech2Write_Backup_\(formatter.string(from: date)).json"
    }
