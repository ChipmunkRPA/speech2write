...
extension SettingsStore {
        ...
        do {
            ...
            if self.launchAtStartupEnabled != enabled {
                let mismatchMessage = enabled
                    ? "macOS did not enable Speech2Write in Login Items. Check System Settings > General > Login Items."
                    : "macOS still shows Speech2Write in Login Items. Check System Settings > General > Login Items."
    ...
    private func launchAtStartupFailureMessage(for error: Error, enabling: Bool) -> String {
        ...
        {
            return "Speech2Write could not \(action) launch at startup. This build may not be signed correctly for macOS Login Items."
        }

        if lowercasedDescription.contains("approval") ||
            lowercasedDescription.contains("authorize")
        {
            return "macOS needs approval before Speech2Write can \(action) launch at startup. Check System Settings > General > Login Items."
        }

        return "Speech2Write could not \(action) launch at startup. macOS reported: \(nsError.localizedDescription)"
    }
    ...
    private var compatibilityLoginItemName: String {
        Bundle.main.object(forInfoDictionaryKey: "CFBundleName") as? String ?? "Speech2Write"
    }
...
private enum LaunchAtStartupSystemState {
    ...
    var message: String {
        switch self {
        case .enabled:
            return "Speech2Write reflects the actual macOS login item state."
        case .disabled:
            return "Speech2Write reflects the actual macOS login item state. Unsigned or development builds may fail to enable this."
        case .requiresApproval:
            return "macOS requires approval for Speech2Write in Login Items before launch at startup becomes active."
        }
    }
}
#endif
