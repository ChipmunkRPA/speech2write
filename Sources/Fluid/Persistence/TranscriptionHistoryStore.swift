...
final class TranscriptionHistoryStore: ObservableObject {
    ...
    /// Delete entries older than the configured retention window (SR-5829 Q6).
    /// Runs at store load, after each append, and when the retention setting
    /// changes. Deletion goes through `deleteEntries(ids:)` so saved audio is
    /// removed the same way as a manual delete and no orphaned files remain.
    @discardableResult
    func purgeExpiredEntries(now: Date = Date()) -> Int {
        guard let retentionDays = SettingsStore.shared.historyRetention.retentionDays,
              let cutoff = Calendar.current.date(byAdding: .day, value: -retentionDays, to: now)
        else {
            return 0
        }

        let expiredIDs = Set(self.entries.filter { $0.timestamp < cutoff }.map { $0.id })
        guard !expiredIDs.isEmpty else { return 0 }

        self.deleteEntries(ids: expiredIDs)
        DebugLogger.shared.info(
            "Purged \(expiredIDs.count) history entries older than \(retentionDays)-day retention",
