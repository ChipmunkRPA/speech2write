...
final class SettingsStore: ObservableObject {
    ...
    private(set) var launchAtStartupStatusMessage =
        "Speech2Write reflects the actual macOS login item state. Unsigned or development builds may fail to enable this."
    ...
    private init() {
        ...
        self.seedBuiltInDictationPromptPresetsIfNeeded()
    ...
    /// Built-in Auto Cleanup presets, shipped as one-click dictation prompt
    /// profiles. Fixed IDs keep seeding idempotent; deleting a preset is
    /// respected (seeding runs once). "None" maps to the existing Off selection.
    static let builtInDictationPromptPresets: [(id: String, name: String, prompt: String)] = [
        (
            id: "builtin-preset-light-cleanup",
            name: "Preset: Light cleanup",
            prompt: """
            ## Cleanup level: Light
            - Fix punctuation and capitalization only.
            - Remove filler words (um, uh, like, you know) and false starts.
            - Do not change wording, grammar, or sentence structure.
            - Keep every remaining word exactly as spoken.
            """
        ),
        (
            id: "builtin-preset-medium-cleanup",
            name: "Preset: Medium cleanup",
            prompt: """
            ## Cleanup level: Medium
            - Fix punctuation and capitalization.
            - Remove filler words (um, uh, like, you know) and false starts.
            - Fix grammatical errors: verb tense, subject-verb agreement, articles.
            - Keep the speaker's wording and sentence structure; do not rephrase.
            """
        ),
        (
            id: "builtin-preset-high-cleanup",
            name: "Preset: High cleanup",
            prompt: """
            ## Cleanup level: High
            - Fix punctuation, capitalization, and grammar.
            - Remove filler words (um, uh, like, you know) and false starts.
            - Tighten wordy or repetitive phrasing while preserving meaning.
            - Restructure run-on sentences into clear, well-formed sentences.
            """
        ),
        (
            id: "builtin-preset-polish",
            name: "Preset: Polish",
            prompt: """
            ## Cleanup level: Polish
            - Fix punctuation, capitalization, and grammar.
            - Remove filler words (um, uh, like, you know) and false starts.
            - Tighten phrasing and restructure run-on sentences.
            - Adjust wording to a clear, professional tone while preserving meaning.
            """
        ),
    ]
    ...
    func effectiveDictationSystemPrompt(for slot: DictationShortcutSlot, appBundleID: String? = nil) -> String {
        let prompt = self.resolvedDictationSystemPrompt(for: slot, appBundleID: appBundleID)
        return self.applyingDictationToneStyle(to: prompt, appBundleID: appBundleID)
    }

    private func resolvedDictationSystemPrompt(for slot: DictationShortcutSlot, appBundleID: String?) -> String {
        if self.promptRoutingScope(for: .dictate) == .selectedAppsOnly {
            guard self.dictationPromptSelection(for: slot) != .off else { return "" }
    ...
    // MARK: - Tone by App Category

    enum DictationAppCategory: String, CaseIterable, Identifiable {
        case email
        case workMessaging
        ...
        static func category(forAppBundleID bundleID: String?) -> DictationAppCategory {
            guard let bundleID = bundleID?.lowercased(), !bundleID.isEmpty else { return .other }

            let emailMarkers = ["mail", "outlook", "thunderbird", "airmail", "spark"]
            if emailMarkers.contains(where: { bundleID.contains($0) }) { return .email }

            let workMessagingMarkers = ["slack", "teams", "zoom"]
            if workMessagingMarkers.contains(where: { bundleID.contains($0) }) { return .workMessaging }

            let personalMessagingMarkers = ["messages", "mobilesms", "telegram", "whatsapp", "signal", "discord"]
            if personalMessagingMarkers.contains(where: { bundleID.contains($0) }) { return .personalMessaging }

            return .other
        }
    }

    enum DictationToneStyle: String, CaseIterable, Identifiable {
        case `default`
        case formal
        case neutral
        case casual
        ...
        var styleLine: String? {
            ...
            case .default:
                return nil
            case .formal:
                return "Match the output to a formal, professional tone: complete sentences, precise wording, no slang."
            case .neutral:
                return "Match the output to a clear, neutral tone: plain wording, no slang, no embellishment."
            case .casual:
                return "Match the output to a relaxed, casual tone: contractions are fine, keep it conversational."
            }
        }
    }

    func dictationToneStyle(for category: DictationAppCategory) -> DictationToneStyle {
        guard let stored = self.defaults.dictionary(forKey: Keys.dictationAppCategoryToneStyles) as? [String: String],
              let raw = stored[category.rawValue],
              let style = DictationToneStyle(rawValue: raw)
        else {
            return .default
        }
        return style
    ...
    func setDictationToneStyle(_ style: DictationToneStyle, for category: DictationAppCategory) {
        objectWillChange.send()
        var stored = (self.defaults.dictionary(forKey: Keys.dictationAppCategoryToneStyles) as? [String: String]) ?? [:]
        if style == .default {
            stored.removeValue(forKey: category.rawValue)
        } else {
            stored[category.rawValue] = style.rawValue
        }
        self.defaults.set(stored, forKey: Keys.dictationAppCategoryToneStyles)
    }

    /// Appends the configured app-category tone line to a resolved dictation
    /// system prompt. Explicit per-app prompt bindings take precedence: apps
    /// with a binding are left untouched. Empty prompts (enhancement off) are
    /// never modified so gating semantics stay intact.
    private func applyingDictationToneStyle(to prompt: String, appBundleID: String?) -> String {
        guard prompt.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty == false else { return prompt }
        guard let appBundleID, !appBundleID.isEmpty else { return prompt }
        guard self.appPromptBinding(for: .dictate, appBundleID: appBundleID) == nil else { return prompt }

        let category = DictationAppCategory.category(forAppBundleID: appBundleID)
        guard let styleLine = self.dictationToneStyle(for: category).styleLine else { return prompt }
    ...
    /// One-time out-of-box seeding: on Macs where Apple Intelligence is available,
    /// pre-select it as the dictation enhancement provider so enhancement works
    /// with no account and no API key, entirely on-device.
    ///
    /// Replicates exactly the state a manual selection produces in AI settings
    /// (select Apple Intelligence, Verify, built-in default prompt — see
    /// `AIEnhancementSettingsViewModel.verifyAppleIntelligence`). If Apple
    /// Intelligence is unavailable at launch the seed flag stays unset so the
    /// seed retries on the next launch — the user may enable Apple Intelligence
    /// in System Settings later. Any explicit provider configuration latches the
    /// flag so the seed never overrides a deliberate choice, even if that
    /// configuration is later removed.
    ///
    /// Called from `AppDelegate.applicationDidFinishLaunching` after
    /// `bootstrapOnboardingState` so the seeded provider selection is not
    /// mistaken for a legacy-usage signal that would suppress onboarding
    /// (see `hasLegacyUsageSignals`).
    func seedAppleIntelligenceEnhancementIfNeeded() {
        guard self.defaults.bool(forKey: Keys.appleIntelligenceAutoSeeded) == false else { return }

        let hasExistingProviderConfiguration =
            self.defaults.object(forKey: Keys.selectedProviderID) != nil ||
            !self.verifiedProviderFingerprints.isEmpty ||
            !self.savedProviders.isEmpty
        if hasExistingProviderConfiguration {
            // The user already configured a provider on their own. Latch the flag
            // so that state is never overridden by a later seed, even if the user
            // deletes the provider again.
            self.defaults.set(true, forKey: Keys.appleIntelligenceAutoSeeded)
            return
        }

        guard AppleIntelligenceService.isAvailable else {
            DebugLogger.shared.info(
                "Apple Intelligence unavailable; skipped dictation enhancement auto-seed (will retry next launch)",
                source: "SettingsStore"
            )
            return
        }

        // Mirror a manual selection: provider + model + verification fingerprint
        // + the built-in default dictation prompt for the primary shortcut.
        self.defaults.set(true, forKey: Keys.appleIntelligenceAutoSeeded)
        self.selectedProviderID = "apple-intelligence"
        var selectedModels = self.selectedModelByProvider
        selectedModels["apple-intelligence"] = "System Model"
        self.selectedModelByProvider = selectedModels
        var fingerprints = self.verifiedProviderFingerprints
        fingerprints["apple-intelligence"] = "apple-intelligence"
        self.verifiedProviderFingerprints = fingerprints
        self.setDictationPromptSelection(.default, for: .primary)
        DebugLogger.shared.info(
            "Auto-selected Apple Intelligence for dictation enhancement",
            source: "SettingsStore"
        )
    }

    /// Records that the user explicitly configured, changed, or disabled a
    /// dictation enhancement provider or prompt selection. Latches the Apple
    /// Intelligence auto-seed flag so the one-time seed never overrides an
    /// explicit choice on a later launch.
    func markDictationEnhancementExplicitlyConfigured() {
        guard self.defaults.bool(forKey: Keys.appleIntelligenceAutoSeeded) == false else { return }
        self.defaults.set(true, forKey: Keys.appleIntelligenceAutoSeeded)
    }

    /// 1.1.2 migration: unwinds the retired 1.1.0/1.1.1 Apple Intelligence
    /// auto-seed. Only installs still in the exact seeded state (provider,
    /// fingerprint, and default prompt untouched) are reverted to raw
    /// dictation; explicit user configurations are left alone. The seeded
    /// flag stays set so the seed can never return.
    func revertAppleIntelligenceAutoSeedIfActive() {
        guard self.defaults.bool(forKey: Keys.appleIntelligenceAutoSeeded),
              self.defaults.bool(forKey: Keys.appleIntelligenceAutoSeedReverted) == false else { return }
        self.defaults.set(true, forKey: Keys.appleIntelligenceAutoSeedReverted)

        guard self.selectedProviderID == "apple-intelligence",
              self.verifiedProviderFingerprints["apple-intelligence"] == "apple-intelligence",
              self.dictationPromptSelection(for: .primary) == .default
        else { return }

        self.defaults.removeObject(forKey: Keys.selectedProviderID)
        var fingerprints = self.verifiedProviderFingerprints
        fingerprints.removeValue(forKey: "apple-intelligence")
        self.verifiedProviderFingerprints = fingerprints
        var selectedModels = self.selectedModelByProvider
        selectedModels.removeValue(forKey: "apple-intelligence")
        self.selectedModelByProvider = selectedModels
        self.setDictationPromptSelection(.off, for: .primary)
        DebugLogger.shared.info(
            "Reverted the Apple Intelligence dictation auto-seed; enhancement is opt-in again",
            source: "SettingsStore"
        )
    }

    /// 1.1.4 migration: Speech2Write is local-only. One-time cleanup that
    /// removes every cloud provider (OpenAI, Anthropic, xAI, Groq, Cerebras,
    /// Google, OpenRouter, and custom endpoints) from the provider selection,
    /// the Rewrite Mode selection, saved providers, and
    /// verification fingerprints. If the active selection was a cloud
    /// provider, the primary dictation prompt selection is turned off so
    /// dictation falls back to raw transcription instead of failing.
    /// API keys already stored in the macOS Keychain are left untouched;
    /// they are simply no longer used.
    func removeCloudProvidersIfNeeded() {
        guard self.defaults.bool(forKey: Keys.cloudProvidersRemoved) == false else { return }
        self.defaults.set(true, forKey: Keys.cloudProvidersRemoved)

        let allowedIDs = Set(ModelRepository.allowedProviderIDs)

        var removedSummary: [String] = []

        // Clear a non-allowlisted global selection and turn primary enhancement off.
        let rawSelection = (self.defaults.string(forKey: Keys.selectedProviderID) ?? "")
            .trimmingCharacters(in: .whitespacesAndNewlines)
        if !rawSelection.isEmpty, !allowedIDs.contains(rawSelection) {
            self.defaults.removeObject(forKey: Keys.selectedProviderID)
            self.setDictationPromptSelection(.off, for: .primary)
            removedSummary.append("selection=\(rawSelection)")
        }

        // Clear a non-allowlisted Rewrite Mode selection.
        let rewriteSelection = self.rewriteModeSelectedProviderID
        if !rewriteSelection.isEmpty, !allowedIDs.contains(rewriteSelection) {
            self.rewriteModeSelectedProviderID = ""
            removedSummary.append("rewriteMode=\(rewriteSelection)")
        }

        // Purge saved custom providers (arbitrary endpoints are no longer supported).
        let staleSavedProviders = self.savedProviders.filter { !allowedIDs.contains($0.id) }
        if !staleSavedProviders.isEmpty {
            self.savedProviders = self.savedProviders.filter { allowedIDs.contains($0.id) }
            removedSummary.append("savedProviders=\(staleSavedProviders.count)")
        }

        // Purge verification fingerprints for anything not allowlisted.
        var fingerprints = self.verifiedProviderFingerprints
        let staleFingerprintKeys = fingerprints.keys.filter { !allowedIDs.contains($0) }
        if !staleFingerprintKeys.isEmpty {
            for key in staleFingerprintKeys {
                fingerprints.removeValue(forKey: key)
            }
            self.verifiedProviderFingerprints = fingerprints
            removedSummary.append("fingerprints=\(staleFingerprintKeys.count)")
        }

        guard !removedSummary.isEmpty else { return }
        DebugLogger.shared.info(
            "Removed cloud AI providers (local-only build): \(removedSummary.joined(separator: ", "))",
            source: "SettingsStore"
        )
    }

    /// 1.1.5 migration: Apple Intelligence is the only enhancement provider.
    /// One-time cleanup that removes the retired local-server providers
    /// (Ollama, LM Studio) from the provider selection, the Rewrite Mode
    /// selection, saved providers, and verification
    /// fingerprints. If the active selection was a local-server provider,
    /// the primary dictation prompt selection is turned off so dictation
    /// falls back to raw transcription instead of failing. Mirrors
    /// removeCloudProvidersIfNeeded and reads the same central allowlist
    /// (ModelRepository.allowedProviderIDs) so the two migrations stay
    /// consistent.
    func removeLocalServerProvidersIfNeeded() {
        guard self.defaults.bool(forKey: Keys.localServerProvidersRemoved) == false else { return }
        self.defaults.set(true, forKey: Keys.localServerProvidersRemoved)
        ...
        // Purge saved providers that are no longer allowlisted (Ollama, LM Studio).
        let staleSavedProviders = self.savedProviders.filter { !allowedIDs.contains($0.id) }
        if !staleSavedProviders.isEmpty {
            self.savedProviders = self.savedProviders.filter { allowedIDs.contains($0.id) }
            removedSummary.append("savedProviders=\(staleSavedProviders.count)")
        ...
        guard !removedSummary.isEmpty else { return }
        DebugLogger.shared.info(
            "Removed local-server AI providers (Apple Intelligence only): \(removedSummary.joined(separator: ", "))",
            source: "SettingsStore"
        )
    }
    ...
    /// Ending a dictation with "press enter"/"hit enter" strips the phrase and
    /// presses Return after the text is inserted (default: ON).
    var autoSendOnPressEnter: Bool {
        get {
            let value = self.defaults.object(forKey: Keys.autoSendOnPressEnter)
            if value == nil { return true }
            return self.defaults.bool(forKey: Keys.autoSendOnPressEnter)
        }
        set {
            objectWillChange.send()
            self.defaults.set(newValue, forKey: Keys.autoSendOnPressEnter)
        }
    }
    ...
    enum AccentColorOption: String, CaseIterable, Identifiable, Codable {
        ...
        var hex: String {
            switch self {
            case .cyan: return "#0A85EA"
            case .green: return "#1FA077"
            case .blue: return "#088181"
            case .purple: return "#7646EC"
            case .orange: return "#BA7D17"
            }
        }
    }
    ...
    enum TranscriptionStartSound: String, CaseIterable, Identifiable, Codable {
        ...
        var displayName: String {
            ...
            case .fluidSfx0: return "Flow SFX 0"
            case .fluidSfx1: return "Flow SFX 1"
            case .fluidSfx2: return "Flow SFX 2"
            case .fluidSfx3: return "Flow SFX 3"
            case .fluidSfx4: return "Flow SFX 4"
            }
        }
    ...
    var accentColor: Color {
        Color(hex: self.accentColorOption.hex) ?? Color(red: 0.039, green: 0.522, blue: 0.918)
    }
    ...
    /// How long dictation history entries (and their saved audio) are kept
    /// before automatic deletion. Raw value is the retention window in days;
    ...
    enum HistoryRetention: Int, CaseIterable, Identifiable {
        ...
        /// Retention window in days; nil means entries are kept forever.
        var retentionDays: Int? {
            self == .forever ? nil : self.rawValue
        }
    }

    /// Retention policy for dictation history. Defaults to 90 days (SR-5829 Q6).
    var historyRetention: HistoryRetention {
        get {
            // object(forKey:) rather than integer(forKey:) so an absent key falls
            // back to the 90-day default instead of reading as 0 (= forever).
            guard let raw = self.defaults.object(forKey: Keys.historyRetentionDays) as? Int,
        ...
        set {
            objectWillChange.send()
            self.defaults.set(newValue.rawValue, forKey: Keys.historyRetentionDays)
        }
    }
    ...
    func makeBackupPayload() -> SettingsBackupPayload {
        SettingsBackupPayload(
            ...
            historyRetentionDays: self.historyRetention.rawValue,
            ...
            enabledVocabularyPresetIDs: self.enabledVocabularyPresetIDs.sorted(),
    ...
    func restore(
        ...
        // Local-only build: never restore cloud/custom providers from a backup.
        self.savedProviders = payload.savedProviders.filter {
            ModelRepository.shared.isAllowedProvider($0.id)
        }
        self.selectedProviderID = ModelRepository.shared.isAllowedProvider(payload.selectedProviderID)
        ...
        self.rewriteModeSelectedProviderID = ModelRepository.shared.isAllowedProvider(payload.rewriteModeSelectedProviderID)
            ? payload.rewriteModeSelectedProviderID
            : ""
        ...
        if let historyRetentionDays = payload.historyRetentionDays {
            self.historyRetention = HistoryRetention(rawValue: historyRetentionDays) ?? .ninetyDays
        }
        ...
        if let enabledVocabularyPresetIDs = payload.enabledVocabularyPresetIDs {
            self.enabledVocabularyPresetIDs = Set(enabledVocabularyPresetIDs)
        }
    ...
    private func seedBuiltInDictationPromptPresetsIfNeeded() {
        guard self.defaults.bool(forKey: Keys.builtInPromptPresetsSeeded) == false else { return }
        self.defaults.set(true, forKey: Keys.builtInPromptPresetsSeeded)

        var profiles = self.dictationPromptProfiles
        let existingIDs = Set(profiles.map(\.id))
        let now = Date()
        for preset in Self.builtInDictationPromptPresets where existingIDs.contains(preset.id) == false {
        ...
        DebugLogger.shared.info("Seeded built-in Auto Cleanup prompt presets", source: "SettingsStore")
    }
    ...
    private func normalizePromptSelectionsIfNeeded() {
        ...
        normalizedProfiles.removeAll { profile in
            ...
            // Literal name of the retired private-AI provider; kept so legacy
            // accidental profiles created by earlier builds are still pruned.
            let isAccidentalPrivateAIProfile = profile.mode.normalized == .dictate &&
                name.caseInsensitiveCompare("Private AI Provider") == .orderedSame &&
                prompt.isEmpty
    ...
    /// IDs of the built-in domain vocabulary packs selected by the user.
    /// Unknown IDs are discarded so renamed or retired presets cannot linger.
    var enabledVocabularyPresetIDs: Set<String> {
        get {
            let stored = self.defaults.stringArray(forKey: Keys.enabledVocabularyPresetIDs) ?? []
            return VocabularyPresetCatalog.normalizedIDs(stored)
        }
        set {
            let normalized = VocabularyPresetCatalog.normalizedIDs(newValue)
            objectWillChange.send()
            self.defaults.set(normalized.sorted(), forKey: Keys.enabledVocabularyPresetIDs)
            NotificationCenter.default.post(name: .parakeetVocabularyDidChange, object: nil)
        }
    }
    ...
    /// Replaces the retired provider + whisper-size dual-setting (raw keys still migrated).
    enum SpeechModel: String, CaseIterable, Identifiable, Codable {
        ...
        var displayName: String {
            ...
            // Qwen support removed; the case stays for stored-settings compatibility
            // and folds into a neighboring retired-engine arm in exhaustive switches.
            case .qwen3Asr, .cohereTranscribeSixBit: return "Cohere Transcribe"
        ...
        var languageSupport: String {
            ...
            case .qwen3Asr, .cohereTranscribeSixBit: return "14 Languages (Select Manually)"
        ...
        var downloadSize: String {
            ...
            case .qwen3Asr, .cohereTranscribeSixBit: return "~1.54 GiB"
        ...
        var expectedDownloadBytes: Int64 {
            ...
            case .qwen3Asr, .cohereTranscribeSixBit: return 1_650_748_785
        ...
        var requiresAppleSilicon: Bool {
            switch self {
            case .parakeetTDT, .parakeetTDTv2, .parakeetRealtime, .cohereTranscribeSixBit, .nemotronOffline, .nemotronStreaming, .nemotronStreaming320: return true
        ...
        var requiresMacOS15: Bool {
            switch self {
            case .cohereTranscribeSixBit: return true
            default: return false
            }
        }

        /// Returns models available for the current Mac's architecture and OS
        /// Speech2Write ships exactly two engines: Parakeet TDT v2 (fastest
        /// local, English) and Apple Speech (macOS 26+, multilingual). The other
        /// upstream engines stay in the enum for stored-settings compatibility
        /// but are hidden from every picker, onboarding route, and download.
        static let enabledModels: [SpeechModel] = [.parakeetTDTv2, .appleSpeechAnalyzer]

        static var availableModels: [SpeechModel] {
            enabledModels.filter { model in
        ...
        static var defaultModel: SpeechModel {
            CPUArchitecture.isAppleSilicon ? .parakeetTDTv2 : .appleSpeech
        }
        ...
        var humanReadableName: String {
            ...
            case .qwen3Asr, .cohereTranscribeSixBit: return "Cohere - High Accuracy"
            case .nemotronOffline: return "Nemotron 3.5 Multilingual"
            case .nemotronStreaming: return "Nemotron Speech 3.5 - Ultra Fast Low Latency"
            case .nemotronStreaming320: return "Nemotron Speech 3.5 - Ultra Fast Low Latency"
            case .appleSpeech: return "Apple ASR Legacy"
        ...
        var cardDescription: String {
            ...
            case .appleSpeechAnalyzer:
                return "Advanced on-device recognition for newer macOS devices. Supports English and many other languages."
            case .whisperTiny:
        ...
        /// Label for the language row on engine cards ("Language" or "Languages")
        var languageRowLabel: String {
            switch self {
        ...
        /// Value for the language row on engine cards
        var languageRowValue: String {
            switch self {
            case .parakeetTDTv2, .parakeetRealtime:
                return "English only"
            case .appleSpeechAnalyzer:
                let otherLanguageCount = VoiceEngineLanguageCatalog.supportedLanguageCount(for: self) - 1
                return otherLanguageCount > 0 ? "English + \(otherLanguageCount) others" : "English + many others"
            default:
                return self.languageSupport
            }
        }
        ...
        var speedRating: Int {
            ...
            case .qwen3Asr, .cohereTranscribeSixBit: return 3
        ...
        var accuracyRating: Int {
            ...
            case .parakeetTDT: return 5
            case .parakeetTDTv2: return 5
            case .parakeetRealtime, .qwen3Asr: return 4
        ...
        var speedPercent: Double {
            ...
            case .qwen3Asr, .cohereTranscribeSixBit: return 0.85
        ...
        var accuracyPercent: Double {
            ...
            case .qwen3Asr, .nemotronOffline: return 0.90
        ...
        /// Optional badge text for the card (e.g., "Speech2Write Pick")
        var badgeText: String? {
            switch self {
            case .parakeetTDT: return "Speech2Write Pick"
            case .parakeetTDTv2: return "Speech2Write Pick"
        ...
        var appleSiliconOptimized: Bool {
            switch self {
            case .parakeetTDT, .parakeetTDTv2, .parakeetRealtime, .cohereTranscribeSixBit, .nemotronOffline, .nemotronStreaming, .nemotronStreaming320, .appleSpeechAnalyzer:
                return true
            default:
                return false
            }
        ...
        var provider: Provider {
            ...
            case .appleSpeech, .appleSpeechAnalyzer:
                return .apple
            // Qwen engine removed; hidden legacy case maps to a retained provider.
            case .qwen3Asr, .cohereTranscribeSixBit:
                return .cohere
            case .whisperTiny, .whisperBase, .whisperSmall, .whisperMedium, .whisperLargeTurbo, .whisperLarge:
                return .openai
        ...
        var isInstalled: Bool {
            ...
            case .cohereTranscribeSixBit, .nemotronOffline, .nemotronStreaming, .nemotronStreaming320:
                // Cohere/Nemotron engines removed; retired models always report not installed.
                return false
            default:
                // Whisper models
...
private extension SettingsStore {
    ...
    enum Keys {
        ...
        static let appleIntelligenceAutoSeeded = "AppleIntelligenceAutoSeededV1"
        static let appleIntelligenceAutoSeedReverted = "AppleIntelligenceAutoSeedRevertedV1"
        static let cloudProvidersRemoved = "CloudProvidersRemovedV1"
        static let localServerProvidersRemoved = "LocalServerProvidersRemovedV1"
        ...
        static let autoSendOnPressEnter = "AutoSendOnPressEnter"
        ...
        static let enabledVocabularyPresetIDs = "EnabledVocabularyPresetIDs"

        // Legacy Transcription Provider (ASR) keys — still read by the
        // first-run migration in migrateToSpeechModel(); never written anymore.
        ...
        static let builtInPromptPresetsSeeded = "BuiltInPromptPresetsSeededV1"
        ...
        // Tone by App Category (dictation style layer)
        static let dictationAppCategoryToneStyles = "DictationAppCategoryToneStyles"
...
extension SettingsStore {
    ...
    /// This unified setting replaces the retired provider + whisper-size combination.
    var selectedSpeechModel: SpeechModel {
        get {
            ...
            {
                // Engines outside the shipped allowlist, or unavailable on
                // this machine, transparently fall back to the default. The
                // stored value is preserved in case a later build re-enables it.
    ...
    /// Migrates the legacy provider + whisper-size dual-setting to the unified SpeechModel
    private func migrateToSpeechModel() -> SpeechModel {
        ...
        case "fluidAudio":
            newModel = CPUArchitecture.isAppleSilicon ? .parakeetTDTv2 : .whisperBase
        default: // "auto"
            newModel = SpeechModel.defaultModel
        }

        // Persist the migrated value RAW (hasLegacyUsageSignals reads this key,
        // and a later build could re-enable a hidden engine).
        ...
        // Return through the same allowlist coercion the stored-value path uses,
        // so a legacy whisper/Intel-fluidAudio upgrade can never surface a
        // retired engine on the first-ever read.
        if !SpeechModel.availableModels.contains(newModel) {
            return SpeechModel.defaultModel
        }
        return newModel
    }
