...
extension View {
    ...
    func fluidButton(
        ...
        case .destructive:
            self.buttonStyle(AccentButtonStyle(compact: size.accentCompact, isDestructive: true))
        case .inline:
    ...
    private struct PrimaryButton: View {
            ...
            if self.isRecording {
                return LinearGradient(
                    colors: [
                        ...
                        self.theme.palette.recording.opacity(0.8),
                    ],
                    startPoint: .topLeading,
                    endPoint: .bottomTrailing
                )
        ...
        var body: some View {
            self.configuration.label
                ...
                .shadow(
                    color: (self.isRecording ? self.theme.palette.recording : self.theme.palette.accent)
...
struct AccentButtonStyle: ButtonStyle {
    ...
    private struct AccentButton: View {
        ...
        var body: some View {
            let tone = self.tone ?? (self.isDestructive ? self.theme.palette.error : self.theme.palette.accent)
            self.configuration.label
                .fontWeight(.semibold)
