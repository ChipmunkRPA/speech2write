...
struct FluidOnboardingLandingHero<Actions: View>: View {
    ...
    var body: some View {
        ...
        .padding(.vertical, self.theme.metrics.onboardingSurface.landing.heroPadding)
        .frame(maxWidth: .infinity, alignment: .center)
        .overlay(alignment: .top) {
            FluidOnboardingHolographicHairline()
        }
...
private final class LandingPrimaryNSButton: NSButton {
    private static let gradientStartColor = NSColor(srgbRed: 66 / 255, green: 240 / 255, blue: 205 / 255, alpha: 1.0)
    private static let gradientEndColor = NSColor(srgbRed: 13 / 255, green: 173 / 255, blue: 242 / 255, alpha: 1.0)
    private static let inkColor = NSColor(srgbRed: 17 / 255, green: 17 / 255, blue: 18 / 255, alpha: 1.0)
    private var trackingArea: NSTrackingArea?
    ...
    override func draw(_ dirtyRect: NSRect) {
        ...
        let angle: CGFloat = self.isFlipped ? -57 : 57

        NSGraphicsContext.saveGraphicsState()
        path.addClip()
        NSGradient(starting: Self.gradientStartColor, ending: Self.gradientEndColor)?
            .draw(in: self.bounds, angle: angle)
        if self.isHighlighted || self.isHovering {
            Self.inkColor.withAlphaComponent(0.08).setFill()
            self.bounds.fill()
        }
    ...
    func update(title: String, isHighlighted: Bool) {
        ...
        self.layer?.shadowColor = Self.gradientEndColor.withAlphaComponent(0.34).cgColor
...
enum FluidOnboardingLandingColors {
    static let blue = Color(red: 10 / 255, green: 133 / 255, blue: 234 / 255)
}

enum FluidOnboardingPrimaryCTAColors {
    static let ink = Color(red: 17 / 255, green: 17 / 255, blue: 18 / 255)
    static let gradient = LinearGradient(
        colors: [
            Color(red: 66 / 255, green: 240 / 255, blue: 205 / 255),
            Color(red: 13 / 255, green: 173 / 255, blue: 242 / 255),
        ],
        startPoint: UnitPoint(x: 0.228, y: 0.919),
        endPoint: UnitPoint(x: 0.772, y: 0.081)
    )
}

private struct FluidOnboardingHolographicHairline: View {
    var body: some View {
        Rectangle()
            .fill(
                LinearGradient(
                    colors: [
                        Color(red: 153 / 255, green: 234 / 255, blue: 255 / 255),
                        Color(red: 179 / 255, green: 167 / 255, blue: 255 / 255),
                        Color(red: 255 / 255, green: 153 / 255, blue: 255 / 255),
                        Color(red: 255 / 255, green: 204 / 255, blue: 153 / 255),
                        Color(red: 153 / 255, green: 255 / 255, blue: 204 / 255),
                    ],
                    startPoint: .leading,
                    endPoint: .trailing
                )
...
private struct OnboardingProminentButtonModifier: ViewModifier {
    ...
    func body(content: Content) -> some View {
        if let controlSize {
            content
                .buttonStyle(FluidOnboardingPrimaryCTAButtonStyle())
...
struct FluidOnboardingPrimaryCTAButtonStyle: ButtonStyle {
    func makeBody(configuration: Configuration) -> some View {
        PrimaryCTAButton(configuration: configuration)
    }
    ...
    private struct PrimaryCTAButton: View {
        ...
        var body: some View {
            let showsInkOverlay = self.configuration.isPressed || self.isHovered
            ...
            self.configuration.label
                ...
                .frame(minHeight: 36)
                .foregroundStyle(FluidOnboardingPrimaryCTAColors.ink)
                .background(
                    self.shape
                        .fill(FluidOnboardingPrimaryCTAColors.gradient)
                        .overlay(
                            self.shape.fill(
                                FluidOnboardingPrimaryCTAColors.ink.opacity(showsInkOverlay ? 0.08 : 0)
                            )
                        )
                )
