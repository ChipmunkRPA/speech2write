...
struct StatsView: View {
    ...
    /// Words dictated per app, sorted descending. Word counting mirrors
    /// TranscriptionHistoryStore's logic over each entry's processed text.
    private var appWordBreakdown: [(app: String, words: Int)] {
        var wordsByApp: [String: Int] = [:]

        for entry in self.historyStore.entries {
            let app = entry.appName.isEmpty ? "Unknown" : entry.appName
            wordsByApp[app, default: 0] += self.wordCount(in: entry.processedText)
        }

        return wordsByApp
            .map { (app: $0.key, words: $0.value) }
    ...
    private var appBreakdownCard: some View {
        ThemedCard(style: .standard, padding: 16, hoverEffect: false) {
            VStack(alignment: .leading, spacing: 12) {
                Label("APP BREAKDOWN", systemImage: "app.fill")
                    .font(.system(size: 11, weight: .semibold))
                    .foregroundStyle(.secondary)

                let breakdown = self.appWordBreakdown.filter { $0.words > 0 }
                ...
                if breakdown.isEmpty {
                    ...
                    let topApps = Array(breakdown.prefix(8))
                    let otherWords = breakdown.dropFirst(8).reduce(0) { $0 + $1.words }
                    let maxWords = topApps.first?.words ?? 1

                    VStack(alignment: .leading, spacing: 8) {
                        ForEach(Array(topApps.enumerated()), id: \.offset) { _, item in
                            self.appBreakdownRow(
                                name: item.app,
                                words: item.words,
                                maxWords: maxWords,
                                isOther: false
    ...
    private func appBreakdownRow(name: String, words: Int, maxWords: Int, isOther: Bool) -> some View {
        HStack(spacing: 10) {
            Text(name)
                .font(.system(size: 11, weight: .medium))
            ...
            GeometryReader { proxy in
                ZStack(alignment: .leading) {
                    ...
                    RoundedRectangle(cornerRadius: 3)
                        .fill(isOther ? Color.secondary.opacity(0.45) : self.theme.palette.accent)
                        .frame(width: max(2, proxy.size.width * CGFloat(words) / CGFloat(max(maxWords, 1))))
                }
            }
            .frame(height: 8)

            Text(self.formatNumber(words))
