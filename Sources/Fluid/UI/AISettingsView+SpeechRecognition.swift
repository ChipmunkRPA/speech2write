                        ...
                        HStack {
                            ...
                            if let badge = model.badgeText {
                                Text(badge)
                                    ...
                                    .padding(.vertical, 2)
                                    .background(Capsule().fill(badge == "Speech2Write Pick" ? .cyan.opacity(0.2) : .orange.opacity(0.2)))
                                    .foregroundStyle(badge == "Speech2Write Pick" ? .cyan : .orange)
                            }
            ...
            VStack(alignment: .leading, spacing: 4) {
                ...
                HStack(spacing: 4) {
                    Image(systemName: "globe")
                        .font(.system(size: 11))
                        .foregroundStyle(self.theme.palette.accent)
                    Text("\(model.languageRowLabel) \(model.languageRowValue)")
                        .font(self.theme.typography.bodyStrong)
                        .foregroundStyle(self.voiceEngineSecondaryText)
                }

                if model == .parakeetTDTv2, self.settings.onboardingSelectedLanguageID != "en" {
                    HStack(spacing: 4) {
                        Image(systemName: "exclamationmark.triangle.fill")
                            .font(.system(size: 11))
                            .foregroundStyle(.orange)
                        Text("Blazing Fast supports English only — keep Apple Speech for your language.")
                            .font(self.theme.typography.bodySmall)
                            .foregroundStyle(.orange)
                    }
                }
