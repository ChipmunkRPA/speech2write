...
final class AIEnhancementSettingsViewModel: ObservableObject {
    ...
    func refreshProviderItems() {
        ...
        for provider in self.savedProviders {
            // Local-only build: custom endpoints are never listed.
            guard ModelRepository.shared.isAllowedProvider(provider.id) else { continue }
            guard !seen.contains(provider.id) else { continue }
            seen.insert(provider.id)
    ...
    func verifyAppleIntelligence() {
        ...
        let key = self.providerKey(for: providerID)
        self.settings.markDictationEnhancementExplicitlyConfigured()
    ...
    func updateCustomProviderBaseURL(_ baseURL: String, for providerID: String) {
        ...
        if !trimmed.isEmpty, !ModelRepository.shared.isLocalhostEndpoint(trimmed) {
            self.updateConnectionStatus(.failed, for: providerID)
            self.setConnectionError("Speech2Write is local-only. Use a localhost address.", for: providerID)
            return
        }
        let current = self.savedProviders[index]
        let updated = SettingsStore.SavedProvider(
    ...
    private func keychainPersistenceExplanation(for error: Error) -> String {
        var message = "Speech2Write could not save the API key to your macOS Keychain, so this provider was not verified."
        ...
        message += "\n\nClick \"Always Allow\" when the Keychain prompt appears, or open Keychain Access > login > Passwords, locate the Speech2Write entry, and grant access."
        return message
    }
    ...
    func presentKeychainAccessAlert(message: String) {
        let msg = message.isEmpty
            ? "Speech2Write stores provider API keys securely in your macOS Keychain. Please grant access by choosing \"Always Allow\" when prompted."
            : message
    ...
    func saveEditedProvider() {
        ...
        // Local-only build: base URLs must point at this Mac.
        if self.selectedProviderID != "apple-intelligence",
           !ModelRepository.shared.isLocalhostEndpoint(base)
        {
            self.updateConnectionStatus(.failed, for: self.selectedProviderID)
            self.setConnectionError("Speech2Write is local-only. Use a localhost address.", for: self.selectedProviderID)
            return
        }
    ...
    func saveNewProvider() {
        ...
        // Local-only build: custom endpoints must point at this Mac.
        guard ModelRepository.shared.isLocalhostEndpoint(base) else { return }
    ...
    func addCurrentAppPromptBinding(for mode: SettingsStore.PromptMode) {
        guard let target = self.resolveBindingTargetApp() else {
            self.appPromptBindingErrorMessage = "Could not detect a target app. Focus another app window (outside Speech2Write) and try again."
    ...
    /// Each shortcut binds independently; forcing a selection would override
