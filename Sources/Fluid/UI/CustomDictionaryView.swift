...
struct CustomDictionaryView: View {
    ...
    @ObservedObject private var correctionStore = CorrectionSuggestionStore.shared
    ...
    private var trainingRecorderDetail: String {
        self.normalizedTrainingReplacement.isEmpty
            ...
            : "Record until Speech2Write recognizes it 3 times in a row. New mishearings are captured below — add the replacement to cover them."
    }
    ...
    private var trainingRecorderStatusText: String {
        ...
        return "\(self.trainingReadinessProgress)/\(CustomDictionaryTrainingMerge.readyCoveredCount) recognized in a row"
    }
    ...
    var body: some View {
        ScrollView(.vertical, showsIndicators: false) {
            VStack(alignment: .leading, spacing: self.theme.metrics.spacing.xl) {
                ...
                VStack(alignment: .leading, spacing: self.theme.metrics.spacing.xxl) {
                    if self.shouldShowSuggestedCorrections {
                        self.suggestedCorrectionsSection
                    }
                    self.trainReplacementSection
                    self.yourDictionarySection
                    self.snippetsSection
                    self.vocabularyBoostingSection
                }
            }
        ...
        .sheet(isPresented: self.$showAddSnippetSheet) {
            AddSnippetSheet(existingTriggers: self.existingSnippetTriggers()) { snippet in
                self.snippetStore.addSnippet(snippet)
            }
        }
        .sheet(item: self.$editingSnippet) { snippet in
            EditSnippetSheet(
                snippet: snippet,
                existingTriggers: self.existingSnippetTriggers(excluding: snippet.id)
            ) { updatedSnippet in
                self.snippetStore.updateSnippet(updatedSnippet)
            }
        }
        .onAppear {
            self.loadBoostTerms()
    ...
    private var shouldShowSuggestedCorrections: Bool {
        !self.correctionStore.pendingSuggestions.isEmpty || !self.correctionStore.isEnabled
    }

    private var correctionLearningBinding: Binding<Bool> {
        Binding(
            get: { self.correctionStore.isEnabled },
            set: { self.correctionStore.isEnabled = $0 }
        )
    }

    private var suggestedCorrectionsSection: some View {
        ThemedCard(style: .standard, hoverEffect: false) {
            if self.correctionStore.isEnabled {
                VStack(alignment: .leading, spacing: self.theme.metrics.spacing.lg) {
                    HStack(alignment: .center, spacing: self.theme.metrics.spacing.md) {
                        self.settingsIconTile(systemName: "wand.and.stars")

                        VStack(alignment: .leading, spacing: 3) {
                            HStack(spacing: 6) {
                                Text("Suggested corrections")
                                    .font(self.theme.typography.sectionTitle)
                                if !self.correctionStore.pendingSuggestions.isEmpty {
                                    Text("(\(self.correctionStore.pendingSuggestions.count))")
                                        .font(self.theme.typography.captionSmall)
                                        .foregroundStyle(self.theme.palette.tertiaryText)
                                }
                            }
                            Text("When you correct dictation right after it types, Speech2Write suggests adding the fix to your dictionary.")
                        ...
                        Toggle("Learn from my corrections", isOn: self.correctionLearningBinding)
                            .font(self.theme.typography.captionStrong)
                            .toggleStyle(.switch)
                            .controlSize(.mini)
                            .help("Suggest dictionary fixes when you correct dictation right after it types.")
                    }

                    VStack(spacing: self.theme.metrics.spacing.sm) {
                        ForEach(self.correctionStore.pendingSuggestions) { suggestion in
                            CorrectionSuggestionRow(
                                suggestion: suggestion,
                                onAccept: { self.acceptCorrectionSuggestion(suggestion) },
                                onDismiss: { self.correctionStore.dismiss(id: suggestion.id) }
                            )
                        }
                    }
                }
                ...
                HStack(alignment: .center, spacing: self.theme.metrics.spacing.md) {
                    Text("Correction learning is off. Speech2Write will not suggest fixes from your edits.")
                        .font(self.theme.typography.caption)
                        .foregroundStyle(self.theme.palette.secondaryText)
    ...
    private func acceptCorrectionSuggestion(_ suggestion: CorrectionSuggestion) {
        let trigger = suggestion.original.lowercased()
        if let index = self.entries.firstIndex(where: { $0.triggers.contains(trigger) }) {
            // The trigger already exists; point it at the accepted correction.
            var entry = self.entries[index]
            entry.replacement = suggestion.corrected
            self.entries[index] = entry
            self.saveEntries()
            self.showReplacementConfirmation(
                title: "Replacement updated",
                detail: "An existing entry now uses this fix."
            )
        } else {
            self.addReplacementEntry(
                SettingsStore.CustomDictionaryEntry(
                    triggers: [suggestion.original],
                    replacement: suggestion.corrected
                )
            )
        }
        self.correctionStore.removeSuggestion(id: suggestion.id)
    }
            ...
            VStack(alignment: .leading, spacing: self.theme.metrics.spacing.lg) {
                HStack(alignment: .center, spacing: self.theme.metrics.spacing.md) {
                    ...
                    VStack(alignment: .leading, spacing: 3) {
                        Text("Teach Words")
                            .font(self.theme.typography.sectionTitle)
                        Text("Show Speech2Write the right spelling, by voice or by typing.")
                            .font(self.theme.typography.caption)
                            .foregroundStyle(self.theme.palette.secondaryText)
                    }
                }
    ...
    private var trainReplacementComposer: some View {
        VStack(alignment: .leading, spacing: self.theme.metrics.spacing.sm) {
            TextField("Type the correct text, e.g. Speech2Write", text: self.$trainingReplacement)
    ...
    private var manualTriggerField: some View {
        VStack(alignment: .leading, spacing: self.theme.metrics.spacing.sm) {
            Text("When Speech2Write hears")
                .font(self.theme.typography.captionStrong)
            TextField("speech to write, speech two write", text: self.$manualTriggersText)
                .textFieldStyle(.roundedBorder)
    ...
    private var manualReplacementField: some View {
        VStack(alignment: .leading, spacing: self.theme.metrics.spacing.sm) {
            Text("Change it to")
                .font(self.theme.typography.captionStrong)
            TextField("Speech2Write", text: self.$manualReplacement)
                .textFieldStyle(.roundedBorder)
                .onSubmit { self.addManualReplacementIfValid() }
            ...
            VStack(alignment: .leading, spacing: self.theme.metrics.spacing.lg) {
                HStack(alignment: .center, spacing: self.theme.metrics.spacing.md) {
                    ...
                    VStack(alignment: .leading, spacing: 3) {
                        ...
                        Text("Words and phrases Speech2Write will correct automatically.")
                            .font(self.theme.typography.caption)
                            .foregroundStyle(self.theme.palette.secondaryText)
                    }
    ...
    private var snippetsSection: some View {
        ThemedCard(style: .standard, hoverEffect: false) {
            VStack(alignment: .leading, spacing: self.theme.metrics.spacing.lg) {
                HStack(alignment: .center, spacing: self.theme.metrics.spacing.md) {
                    self.settingsIconTile(systemName: "text.append")

                    VStack(alignment: .leading, spacing: 3) {
                        HStack(spacing: 6) {
                            Text("Snippets")
                                .font(self.theme.typography.sectionTitle)
                            if !self.snippetStore.snippets.isEmpty {
                                Text("(\(self.snippetStore.snippets.count))")
                                    .font(self.theme.typography.captionSmall)
                                    .foregroundStyle(self.theme.palette.tertiaryText)
                            }
                        }
                        Text("Say the trigger phrase while dictating to insert the snippet.")
                            .font(self.theme.typography.caption)
                            .foregroundStyle(self.theme.palette.secondaryText)
                    }
                ...
                if self.snippetStore.snippets.isEmpty {
                    self.dictionaryEmptyState(
                        ...
                        detail: "Add a trigger phrase and the text it should insert, like a scheduling link and sign-off."
                    ) {
                        self.showAddSnippetSheet = true
                    }
                } else {
                    VStack(spacing: self.theme.metrics.spacing.sm) {
                        ForEach(self.snippetStore.snippets) { snippet in
                            SnippetRow(
                                snippet: snippet,
                                onToggle: { isEnabled in
                                    self.snippetStore.setEnabled(isEnabled, id: snippet.id)
                                },
                                onEdit: { self.editingSnippet = snippet },
                                onDelete: { self.snippetStore.deleteSnippet(id: snippet.id) }
                            )
                        }
                    }
                }
    ...
    private func existingSnippetTriggers(excluding snippetId: UUID? = nil) -> Set<String> {
        var triggers = Set<String>()
        for snippet in self.snippetStore.snippets where snippet.id != snippetId {
        ...
        ThemedCard(style: .standard, hoverEffect: false) {
            VStack(alignment: .leading, spacing: self.theme.metrics.spacing.lg) {
                HStack(alignment: .center, spacing: self.theme.metrics.spacing.md) {
                    ...
                    VStack(alignment: .leading, spacing: 3) {
                        HStack(spacing: 6) {
                            Text("Vocabulary Boosting")
                                .font(self.theme.typography.sectionTitle)
                            if !self.settings.enabledVocabularyPresetIDs.isEmpty {
                                Text("(\(self.settings.enabledVocabularyPresetIDs.count) presets)")
                                    .font(self.theme.typography.captionSmall)
                                    .foregroundStyle(self.theme.palette.tertiaryText)
                            }
                        }
                        Text("Use field presets and custom words to improve Parakeet recognition.")
                            .font(self.theme.typography.caption)
                            .foregroundStyle(self.theme.palette.secondaryText)
                    }
                    ...
                    Toggle("Boosting", isOn: Binding(
                        get: { self.settings.vocabularyBoostingEnabled },
                        set: { self.settings.vocabularyBoostingEnabled = $0 }
                    ))
                    .font(self.theme.typography.captionStrong)
                    .toggleStyle(.switch)
                    .controlSize(.mini)
                    .help("Improve recognition of presets and custom words when using Parakeet.")
                ...
                VStack(alignment: .leading, spacing: self.theme.metrics.spacing.sm) {
                    HStack {
                        Text("Preset Dictionaries")
                            .font(self.theme.typography.bodySmallStrong)

                        Spacer()

                        Text("Optional · stored locally")
                            .font(self.theme.typography.captionSmall)
                            .foregroundStyle(self.theme.palette.tertiaryText)
                    }
                    ...
                    LazyVGrid(
                        columns: [
                            GridItem(.flexible(), spacing: self.theme.metrics.spacing.sm),
                            GridItem(.flexible(), spacing: self.theme.metrics.spacing.sm),
                        ],
                        spacing: self.theme.metrics.spacing.sm
                    ) {
                        ForEach(VocabularyPresetCatalog.all) { preset in
                            self.vocabularyPresetRow(preset)
                        }
                    }
                }
                ...
                HStack {
                    VStack(alignment: .leading, spacing: 2) {
                        Text("Your Custom Words")
                            .font(self.theme.typography.bodySmallStrong)
                        Text("Names, products, and uncommon terms unique to you.")
                            .font(self.theme.typography.caption)
                            .foregroundStyle(self.theme.palette.secondaryText)
                    }
    ...
    private func vocabularyPresetRow(_ preset: VocabularyPreset) -> some View {
        let isEnabled = self.settings.enabledVocabularyPresetIDs.contains(preset.id)
        let shape = RoundedRectangle(cornerRadius: self.theme.metrics.corners.md, style: .continuous)

        return Toggle(isOn: Binding(
            get: { self.settings.enabledVocabularyPresetIDs.contains(preset.id) },
            set: { self.setVocabularyPreset(preset, enabled: $0) }
        )) {
            HStack(spacing: self.theme.metrics.spacing.sm) {
                ZStack {
                    RoundedRectangle(cornerRadius: self.theme.metrics.corners.sm, style: .continuous)
                        .fill(self.theme.palette.accent.opacity(isEnabled ? 0.16 : 0.07))
                    Image(systemName: preset.systemImage)
                        .font(.system(size: 13, weight: .semibold))
                        .foregroundStyle(isEnabled ? self.theme.palette.accent : self.theme.palette.secondaryText)
                }
                .frame(width: 32, height: 32)
                ...
                VStack(alignment: .leading, spacing: 2) {
                    Text(preset.title)
                        .font(self.theme.typography.bodySmallStrong)
                    Text("\(preset.terms.count) terms · \(preset.description)")
                        .font(self.theme.typography.captionSmall)
                        .foregroundStyle(self.theme.palette.secondaryText)
                        .lineLimit(1)
                }
        ...
        .background(
            shape
                .fill(self.theme.palette.contentBackground.opacity(isEnabled ? 0.72 : 0.42))
                .overlay(
                    shape.stroke(
                        isEnabled ? self.theme.palette.accent.opacity(0.35) : self.theme.palette.cardBorder.opacity(0.25),
                        lineWidth: 1
                    )
                )
        )
        .accessibilityHint("Adds or removes this field vocabulary from Parakeet recognition")
    }

    private func setVocabularyPreset(_ preset: VocabularyPreset, enabled: Bool) {
        var selected = self.settings.enabledVocabularyPresetIDs
        if enabled {
            selected.insert(preset.id)
            self.settings.vocabularyBoostingEnabled = true
        } else {
            selected.remove(preset.id)
        }
        self.settings.enabledVocabularyPresetIDs = selected
    }
    ...
    private var boostingInfoPopover: some View {
        VStack(alignment: .leading, spacing: self.theme.metrics.spacing.sm) {
            ...
            Text("Vocabulary Boosting is an experimental feature that helps Parakeet recognize enabled presets and your custom words.")
                .font(self.theme.typography.caption)
                .foregroundStyle(self.theme.palette.secondaryText)

            Text("Preset selections and custom words are stored locally and never uploaded.")
                .font(self.theme.typography.caption)
                .foregroundStyle(self.theme.palette.secondaryText)
...
private enum DictionaryComposerMode: CaseIterable, Identifiable {
    ...
    var detail: String {
        switch self {
        case .train:
            return "Say it a few times so Speech2Write can catch the versions it hears."
        case .manual:
        ...
        ScrollView(.vertical, showsIndicators: false) {
            VStack(alignment: .leading, spacing: 14) {
                ...
                VStack(alignment: .leading, spacing: 6) {
                    ...
                    TextField("Speech2Write", text: self.$termText)
                        .textFieldStyle(.roundedBorder)
        ...
        VStack(alignment: .leading, spacing: 16) {
            ...
            VStack(alignment: .leading, spacing: 6) {
                ...
                Text("Enter words separated by commas. These are what the transcription might hear.")
                    .font(.caption)
                    .foregroundStyle(.secondary)
                TextField("speech to write, speech two write", text: self.$triggersText)
                    .textFieldStyle(.roundedBorder)
                    .onSubmit { self.saveIfValid() }
            ...
            VStack(alignment: .leading, spacing: 6) {
                ...
                Text("This is what will appear in the final transcription.")
                    .font(.caption)
                    .foregroundStyle(.secondary)
                TextField("Speech2Write", text: self.$replacement)
                    .textFieldStyle(.roundedBorder)
                    .onSubmit { self.saveIfValid() }
            }
...
struct SnippetRow: View {
    ...
    var body: some View {
        HStack(alignment: .center, spacing: self.theme.metrics.spacing.sm) {
            VStack(alignment: .leading, spacing: 2) {
                Text(self.snippet.trigger)
                    .font(self.theme.typography.bodySmallStrong)
                    .foregroundStyle(self.snippet.isEnabled ? self.theme.palette.accent : self.theme.palette.secondaryText)
                    .lineLimit(1)
                    .truncationMode(.tail)

                Text(self.snippet.previewText)
                    .font(self.theme.typography.caption)
                    .foregroundStyle(self.theme.palette.secondaryText)
                    .lineLimit(1)
                    .truncationMode(.tail)
            ...
            Toggle("Enabled", isOn: Binding(
                get: { self.snippet.isEnabled },
                set: { self.onToggle($0) }
            ))
            .labelsHidden()
            .toggleStyle(.switch)
            .controlSize(.mini)
            .help(self.snippet.isEnabled ? "Disable snippet" : "Enable snippet")
...
// MARK: - Correction Suggestion Row
...
struct CorrectionSuggestionRow: View {
    ...
    var body: some View {
        HStack(alignment: .center, spacing: self.theme.metrics.spacing.sm) {
            ...
            Text(self.suggestion.corrected)
                .font(self.theme.typography.bodySmallStrong)
                .foregroundStyle(self.theme.palette.accent)
                .lineLimit(1)
                .truncationMode(.tail)

            if self.suggestion.occurrenceCount > 1 {
                Text("\(self.suggestion.occurrenceCount)×")
                    .font(self.theme.typography.captionSmall)
                    .foregroundStyle(self.theme.palette.secondaryText)
                    .padding(.horizontal, 6)
                    .padding(.vertical, 2)
                    .background(Capsule().fill(.quaternary))
                    .help("Seen \(self.suggestion.occurrenceCount) times")
            }

            Spacer(minLength: self.theme.metrics.spacing.sm)

            HStack(spacing: self.theme.metrics.spacing.xs) {
                Button("Accept") { self.onAccept() }
                    .fluidButton(.accent, size: .compact)
                    .help("Add this fix to your dictionary")

                Button("Dismiss") { self.onDismiss() }
                    .fluidButton(.compact, size: .compact)
...
struct AddSnippetSheet: View {
    ...
    private var isDuplicate: Bool {
        self.existingTriggers.contains(self.normalizedTrigger.lowercased())
    }

    private var canSave: Bool {
        !self.normalizedTrigger.isEmpty &&
            !self.expansion.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty &&
            !self.isDuplicate
    }
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 16) {
            ...
            HStack {
                Spacer()
                Button("Add Snippet") { self.saveIfValid() }
                    .buttonStyle(.borderedProminent)
                    .tint(self.theme.palette.accent)
                    .disabled(!self.canSave)
        ...
        .frame(minHeight: 400, idealHeight: 440, maxHeight: 540)
    }
    ...
    private func saveIfValid() {
        ...
        let snippet = Snippet(
            trigger: self.normalizedTrigger,
            expansion: self.expansion
        )
        self.onSave(snippet)
        self.dismiss()
...
struct EditSnippetSheet: View {
    ...
    var body: some View {
        ...
        .frame(minHeight: 400, idealHeight: 440, maxHeight: 540)
        .onAppear {
            self.trigger = self.snippet.trigger
            self.expansion = self.snippet.expansion
        }
    }
    ...
    private func saveIfValid() {
        ...
        let updatedSnippet = Snippet(
            ...
            trigger: self.normalizedTrigger,
            expansion: self.expansion,
            isEnabled: self.snippet.isEnabled,
            createdAt: self.snippet.createdAt
        )
        self.onSave(updatedSnippet)
        self.dismiss()
    }
}

// MARK: - Snippet Sheet Fields

private struct SnippetSheetFields: View {
    ...
    var body: some View {
        ...
        VStack(alignment: .leading, spacing: 6) {
            Text("Trigger phrase")
                .font(.subheadline.weight(.medium))
            Text("Say this phrase while dictating to insert the snippet.")
                .font(.caption)
                .foregroundStyle(.secondary)
            TextField("insert my scheduling link", text: self.$trigger)
                .textFieldStyle(.roundedBorder)
            ...
            if self.isDuplicate {
                HStack(spacing: 4) {
                    Image(systemName: "exclamationmark.triangle.fill")
                        .foregroundStyle(.orange)
                    Text("This trigger phrase is already used by another snippet.")
                        .foregroundStyle(.orange)
                }
                .font(.caption)
            }
        ...
        VStack(alignment: .leading, spacing: 6) {
            Text("Snippet text")
                .font(.subheadline.weight(.medium))
            Text("This replaces the trigger phrase in your transcription. Line breaks are kept.")
                .font(.caption)
                .foregroundStyle(.secondary)

            TextEditor(text: self.$expansion)
                .font(.body)
                ...
                .onChange(of: self.expansion) { _, newValue in
                    if newValue.count > Snippet.maxExpansionLength {
                        self.expansion = String(newValue.prefix(Snippet.maxExpansionLength))
                    }
                }

            Text("\(self.expansion.count)/\(Snippet.maxExpansionLength)")
                .font(.caption)
                .foregroundStyle(self.expansion.count >= Snippet.maxExpansionLength ? AnyShapeStyle(Color.orange) : AnyShapeStyle(.secondary))
