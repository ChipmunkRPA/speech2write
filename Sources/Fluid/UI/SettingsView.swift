...
struct SettingsView: View {
                    ...
                    VStack(alignment: .leading, spacing: 14) {
                        ...
                        VStack(spacing: 16) {
                            ...
                            self.settingsToggleRow(
                                ...
                                description: "Automatically start Speech2Write when you log in",
                            ...
                            self.settingsToggleRow(
                                ...
                                description: "When off, Speech2Write starts silently in the menu bar at login. Opening the app yourself always shows the window.",
                                isOn: Binding(
                            ...
                            self.settingsToggleRow(
                                ...
                                description: "Keep Speech2Write in the menu bar only (hides Dock icon and Cmd+Tab entry)",
                            ...
                            VStack(alignment: .leading, spacing: 6) {
                                VStack(alignment: .leading, spacing: 2) {
                                    Text("Updates")
                                        .font(self.theme.typography.bodyStrong)
                                        .foregroundStyle(self.settingsTitleText)
                                    Text("Speech2Write is an open-source CPA Automation project. Updates are installed manually from GitHub Releases.")
                                        .font(self.theme.typography.bodySmall)
                                        .foregroundStyle(self.settingsSecondaryText)
                                }
                            ...
                            HStack(spacing: 10) {
                                ...
                                Button("cpaautomation.ai") {
                                    self.openProjectWebsite()
                                }
                                .buttonStyle(.bordered)
                                .controlSize(.regular)
                            }
                            ...
                            VStack(alignment: .leading, spacing: 12) {
                                if self.isRecordingAnyShortcut {
                                    ...
                                    HStack(spacing: 8) {
                                        ...
                                        Button("Retry") {
                                            self.hotkeyManagerInitialized = false
                                ...
                                VStack(alignment: .leading, spacing: 8) {
                                    ...
                                    Text("Primary dictation can use a keyboard shortcut or allowed mouse button. Changes apply immediately.")
                                        .font(.caption)
                                ...
                                VStack(spacing: 12) {
                                    ...
                                    HStack(alignment: .center) {
                                        VStack(alignment: .leading, spacing: 2) {
                                            Text("History retention")
                                                .font(self.theme.typography.bodyStrong)
                                                .foregroundStyle(self.settingsTitleText)
                                            Text("Dictation history older than this is deleted automatically.")
                                                .font(self.theme.typography.bodySmall)
                                                .foregroundStyle(self.settingsSecondaryText)
                                        }
                                        ...
                                        Picker("", selection: Binding(
                                            ...
                                            set: {
                                                SettingsStore.shared.historyRetention = $0
                                                TranscriptionHistoryStore.shared.purgeExpiredEntries()
                                                self.refreshAudioHistoryUsage()
                                            }
                                        )) {
                                            ForEach(SettingsStore.HistoryRetention.allCases) { option in
                                                Text(option.displayName).tag(option)
                                            }
                                        }
                                        .pickerStyle(.menu)
                                    ...
                                    self.optionToggleRow(
                                        title: "Say 'press enter' to send",
                                        description: "End a dictation with \"press enter\" or \"hit enter\" to press the Return key after the text is inserted.",
                                        isOn: Binding(
                                            get: { SettingsStore.shared.autoSendOnPressEnter },
                                            set: { SettingsStore.shared.autoSendOnPressEnter = $0 }
                                        )
                                    )
                                    Divider().opacity(0.2)

                                    self.toneByAppCategorySection
                                    Divider().opacity(0.2)

                                    self.optionToggleRow(
                                        title: "Pause Media During Transcription",
                                        description: "Automatically pause currently playing audio/video when transcription starts. Resumes only if Speech2Write paused it.",
                                        isOn: Binding(
    ...
    private func exportBackup() {
        do {
            ...
            self.presentInfoAlert(
                ...
                message: "Saved your Speech2Write backup to:\n\(url.path)"
            )
        } catch {
    ...
    private func applyAudioHistoryBudget() {
        ...
        if self.audioHistoryUsageBytes > newBudgetBytes {
            ...
            confirm.informativeText = """
            This budget is below current audio usage. Speech2Write will delete the oldest saved audio first and keep transcript history.
            """
            confirm.alertStyle = .warning
    ...
    private func openProjectWebsite() {
        guard let url = URL(string: AppBundleMetadata.websiteURLString) else { return }
        NSWorkspace.shared.open(url)
    }
    ...
    private var toneByAppCategorySection: some View {
        VStack(alignment: .leading, spacing: 10) {
            VStack(alignment: .leading, spacing: 2) {
                Text("Tone by App Category")
                    .font(self.theme.typography.bodyStrong)
                    .foregroundStyle(self.settingsTitleText)
                Text("Apply a writing tone based on the kind of app you dictate into. Requires AI enhancement; apps with an explicit prompt assignment are not affected.")
                    .font(self.theme.typography.bodySmall)
                    .foregroundStyle(self.settingsSecondaryText)
                    .fixedSize(horizontal: false, vertical: true)
            }

            ForEach(SettingsStore.DictationAppCategory.allCases) { category in
                HStack(alignment: .center) {
                    ...
                    Picker("", selection: Binding(
                        get: { SettingsStore.shared.dictationToneStyle(for: category) },
                        set: { SettingsStore.shared.setDictationToneStyle($0, for: category) }
                    )) {
                        ForEach(SettingsStore.DictationToneStyle.allCases) { style in
    ...
    private func primaryDictationShortcutsList() -> some View {
        ...
        let defaultShortcut = HotkeyShortcut(keyCode: 61, modifierFlags: [], modifierKeyCodes: [61])
        let isUsingDefault = self.primaryDictationShortcuts == [defaultShortcut]
        ...
        VStack(alignment: .leading, spacing: 6) {
            HStack(spacing: 10) {
                ...
                Button("Reset default") {
                    self.shortcutRecordingMessage = nil
                    self.activeShortcutRecordingTarget = nil
                    self.primaryDictationShortcuts = [defaultShortcut]
                }
                .buttonStyle(.bordered)
                .controlSize(.small)
                .disabled(self.isRecordingAnyShortcut || isUsingDefault)
                .help("Restore Right Option as the only primary dictation shortcut")
