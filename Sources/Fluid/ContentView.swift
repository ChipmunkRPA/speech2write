...
enum ShortcutRecordingTarget: Hashable {
    ...
    var enablesFeatureOnAssignment: Bool {
        switch self {
        case .secondaryDictation, .edit, .pasteLast:
            return true
    ...
    var allowsMouseShortcut: Bool {
        ...
        case .primaryDictation, .pasteLast:
            return true
        case .secondaryDictation, .edit, .cancel, .dictationPrompt, .newPrompt:
            return false
        }
    }
...
struct ContentView: View {
    ...
    private func handleContentAppear() {
        ...
        CorrectionLearningService.shared.beginWatching()
    }
    ...
    private func openIssueReportingPage() {
        guard let url = URL(string: "\(AppBundleMetadata.repositoryURLString)/issues") else { return }
    ...
    private var sidebarView: some View {
        ...
        .navigationTitle(AppBundleMetadata.appDisplayName)
        ...
        VStack(alignment: .leading, spacing: 8) {
            ...
            VStack(alignment: .leading, spacing: 4) {
                if self.asr.micStatus == .notDetermined {
                    ...
                    self.instructionStep(number: "2", text: "Find **Speech2Write** in the microphone list")
                    self.instructionStep(number: "3", text: "Toggle **Speech2Write ON** to allow access")
                }
            }
            .padding(.leading, 4)
        }
    ...
    private func captureRecordingContext() {
        ...
        DebugLogger.shared.debug(
            "Captured recording app context: app=\(info.name), bundleId=\(info.bundleId), titleChars=\(info.windowTitle.count)",
            source: "ContentView"
        )
    ...
    // MARK: - "Press enter" auto-send

    /// Matches a trailing spoken "press enter"/"hit enter" trigger phrase,
    /// including separator punctuation before it (commas, dashes — but not
    /// sentence-ending punctuation) and any trailing punctuation after it.
    private static let autoSendTriggerRegex = try? NSRegularExpression(
        pattern: "[\\s,;:…-]*\\b(press|hit)\\s+enter[\\s.,;:!?…]*$",
        options: .caseInsensitive
    )

    /// Strips a trailing "press enter"/"hit enter" trigger from the final transcript.
    /// Returns the stripped text and whether the trigger was present.
    private static func strippingAutoSendTrigger(from text: String) -> (text: String, triggered: Bool) {
        guard let regex = self.autoSendTriggerRegex else { return (text, false) }
        let nsText = text as NSString
        let fullRange = NSRange(location: 0, length: nsText.length)
        guard let match = regex.firstMatch(in: text, options: [], range: fullRange) else {
            return (text, false)
        }
        return (nsText.substring(to: match.range.location), true)
    }
    ...
    private func processTextWithAI(
        ...
        // This ensures AI provider/model changes in the AI settings take effect immediately
        let currentSelectedProviderID = SettingsStore.shared.selectedProviderID
        let storedSelectedModelByProvider = SettingsStore.shared.selectedModelByProvider
        let storedSavedProviders = SettingsStore.shared.savedProviders

        // Derive the canonical provider key and model from the current settings
        ...
        if let saved = storedSavedProviders.first(where: { $0.id == currentSelectedProviderID }) {
            ...
            // Built-in provider (apple-intelligence)
        ...
        if currentSelectedProviderID == "apple-intelligence" {
            ...
            if #available(macOS 26.0, *) {
                ...
                // The on-device model needs the prompt via the Instructions API —
                // folding it into the user turn makes it intermittently echo the
                // instructions back as output. Placeholder prompts still fold,
                // wrapped with a minimal instruction.
                ...
                if isDictationCall,
                   DictationEnhancementOutputGuard.isLikelyPromptEcho(output: output, transcript: inputText, prompt: promptText) {
                    DebugLogger.shared.warning(
        ...
        // Any other selection is stale — fail closed so dictation falls back
        // to the raw transcription (SR-5829, no generic chat-HTTP client
    ...
    private func stopAndProcessTranscription(route: DictationOutputRoute = .normal) async {
        ...
        DebugLogger.shared.info(
            "Routing decision snapshot | activeMode=\(modeAtStop.rawValue) | rewrite=\(wasRewriteMode) | overlay=\(NotchContentState.shared.mode.rawValue)",
            source: "ContentView"
        )
        ...
        // Edit mode calls asr.stop() without this callback.
        ...
        if wasRewriteMode {
            DebugLogger.shared.info(
                "Processing rewrite [instruction omitted, chars=\(transcribedText.count)]",
                source: "ContentView"
            )
            let appInfo = self.recordingAppInfo ?? self.getCurrentAppInfo()
        ...
        var shouldAutoSendReturn = false
        if SettingsStore.shared.autoSendOnPressEnter {
            let stripped = Self.strippingAutoSendTrigger(from: finalText)
            if stripped.triggered {
                finalText = stripped.text
                shouldAutoSendReturn = true
                DebugLogger.shared.info("Auto-send trigger detected; Return will be pressed after insertion", source: "ContentView")
            }
        }
        ...
        // When the app itself is frontmost, the bound editor already receives `finalText`.
        ...
        if shouldTypeExternally {
            ...
            if shouldAutoSendReturn {
                if finalText.isEmpty {
                    // The dictation was only the trigger phrase; press Return directly.
                    let returnTargetPID = typingTarget.pid
                    DispatchQueue.global(qos: .userInitiated).async {
                        TypingService.postReturnKeystroke(targetPID: returnTargetPID)
                    }
                } else {
                    TypingService.requestReturnKeyAfterNextInsertion()
                }
            }
            ...
            // Correction auto-learning: remember what was typed and where.
            // Held in memory only; used to diff against the field if the user
            // fixes the output right away.
            CorrectionLearningService.shared.recordDictationOutput(
                typedText: finalText,
        ...
        if didTypeExternally {
            ...
            let wordsBucket = PostTranscriptionEditTracker.bucketWords(PostTranscriptionEditTracker.wordCount(in: finalText))
    ...
    private var isOnboardingSandboxRouteActive: Bool {
        let onboardingAIEnhancementStep = 6
    ...
    private func processRewriteWithVoiceInstruction(
        ...
        DebugLogger.shared.info(
            "Processing \(hasOriginalText ? "rewrite" : "write/improve") [instruction omitted, chars=\(instruction.count), originalTextChars=\(self.rewriteModeService.originalText.count)]",
            source: "ContentView"
        )
    ...
    private func startRecording() {
        ...
        // Ensure normal dictation mode is set (rewrite mode sets its own)
...
extension ContentView {
    ...
    func openAccessibilitySettings() {
        ...
        // Ask TCC to register the current signed app before opening the pane. This is
        // especially important after a user replaces a locally built copy of the app.
    ...
    private func positionWindowBesideSystemSettings(requestID: UUID) {
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.45) {
            ...
            guard let window = NSApp.windows.first(where: { $0.isVisible && $0.title == AppBundleMetadata.appDisplayName }) ?? NSApp.keyWindow else {
                return
            }
    ...
    private func cancelAccessibilityPermissionFlow() {
        ...
        (NSApp.windows.first { $0.isVisible && $0.title == AppBundleMetadata.appDisplayName } ?? NSApp.keyWindow)?
    ...
    private var draggableAccessibilityAppURL: URL {
        ...
        let installedURL = URL(fileURLWithPath: "/Applications/Speech2Write.app")
