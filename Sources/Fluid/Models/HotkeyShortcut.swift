...
/// A normalized, replaceable set of shortcuts used by the live hotkey manager.
///
/// Keeping replacement semantics in one small value type prevents a changed shortcut
/// from accidentally leaving its previous binding in the event-routing list.
struct HotkeyShortcutSet: Equatable {
    private(set) var shortcuts: [HotkeyShortcut]

    init(_ shortcuts: [HotkeyShortcut]) {
        self.shortcuts = []
        self.replace(with: shortcuts)
    }

    mutating func replace(with shortcuts: [HotkeyShortcut]) {
        var unique: [HotkeyShortcut] = []
        for shortcut in shortcuts where !unique.contains(shortcut) {
            unique.append(shortcut)
        }
        self.shortcuts = unique
    }

    func keyboardShortcut(keyCode: UInt16, modifiers: NSEvent.ModifierFlags) -> HotkeyShortcut? {
        self.shortcuts.first { $0.matches(keyCode: keyCode, modifiers: modifiers) }
    }

    var modifierOnlyShortcuts: [HotkeyShortcut] {
        self.shortcuts.filter(\.isModifierOnlyShortcut)
    }

    func modifierOnlyShortcut(matching pressedKeyCodes: Set<UInt16>) -> HotkeyShortcut? {
        let normalizedPressed = HotkeyShortcut.normalizedModifierKeyCodes(from: Array(pressedKeyCodes))
        return self.modifierOnlyShortcuts.first { $0.normalizedModifierKeyCodes == normalizedPressed }
    }

    func containsMouseShortcut(button: Int, modifiers: NSEvent.ModifierFlags) -> Bool {
        self.shortcuts.contains { $0.matchesMouse(button: button, modifiers: modifiers) }
    }
}
