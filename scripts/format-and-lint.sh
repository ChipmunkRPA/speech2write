...
ensure_tool() {
  ...
  if [ "${SPEECH2WRITE_NO_TOOL_INSTALL:-0}" = "1" ]; then
    die "Missing dependency: '$tool' (auto-install disabled via SPEECH2WRITE_NO_TOOL_INSTALL=1)."
  fi
...
tmpfile="$(mktemp -t speech2write-swiftlint.XXXXXX)"
